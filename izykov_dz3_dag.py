...
ods_issue = DataProcHiveOperator(
    ...
    """,
    cluster_name='cluster-dataproc',
    job_name = user_name + '_ods_issue_{{ execution_date.year }}_{{ params.job_suffix }}',
    params = {"job_suffix": randint(0, 100000)},
    region = 'europe-west3',
)

ods_payment = DataProcHiveOperator(
    task_id = user_name + '_ods_payment',
    dag = dag,
    query = """
    ...
    """,
    cluster_name='cluster-dataproc',
    job_name = user_name + '_ods_payment_{{ execution_date.year }}_{{ params.job_suffix }}',
    params = {"job_suffix": randint(0, 100000)},
    region = 'europe-west3',
)

ods_traffic = DataProcHiveOperator(
    task_id = user_name + '_ods_traffic',
    dag = dag,
    query = """
        ...
        /*
            Ниже - обновление витрины dm_traffic, можно было вынести в отдельный DAG, например,
            если до 11 января этими данными все равно никто не пользуется - то обновлять 10-го.
    ...
    """,
    cluster_name='cluster-dataproc',
    job_name = user_name + '_ods_traffic_{{ execution_date.year }}_{{ params.job_suffix }}',
    params = {"job_suffix": randint(0, 100000)},
    region = 'europe-west3',
)
